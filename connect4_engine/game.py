...
class Connect4Game:
    ...
    def __init__(
        ...
        self.board = Board()
        solver = "connect4_engine/core/c4solver" + (
            "" if sys.platform == "linux" else ".exe"
        )
    ...
    def piece_dropped_in_board(self, column: int):
        ...
        self.turns_taken[self.turn] += 1  # self.turns_taken should always be 'player' here..
        if self.turn == "ai":
            self.logger.error(
                "board isn't supposed to see ai moves bc they fall under the ledstrip!"
            )
    ...
    def ai_turn(self):
        ...
        self.board.drop_piece(ai_column, Connect4Game.AI_COLOR)
        if self.check_winner():
            return
        self.turn = "player"
        self.robot.give_player_puck(self.turns_taken["player"])
